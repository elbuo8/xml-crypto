  ...
  */
  this.getSignature = async function(signedInfo, signingKey) {
    ...
    const signature = await crypto.subtle.sign(
        { name: ALGORITHM_NAME, saltLength: 20, hash: { name: 'SHA-1' } },
        key,
...
function RSASHA256() {
  ...
  */
  this.getSignature = async function(signedInfo, signingKey) {
    ...
    const signature = await crypto.subtle.sign(
        { name: ALGORITHM_NAME, saltLength: 32, hash: { name: 'SHA-256' } },
        key,
        encoder.encode(signedInfo)
    );
    return Buffer.from(signature).toString('base64');
...
function RSASHA512() {
  ...
  */
  this.getSignature = async function(signedInfo, signingKey) {
    ...
    const signature = await crypto.subtle.sign(
        { name: ALGORITHM_NAME, saltLength: 64, hash: { name: 'SHA-512' } },
        key,
        encoder.encode(signedInfo)
    );
    return Buffer.from(signature).toString('base64');
...
function HMACSHA1() {
  this.getSignature = async function(signedInfo, signingKey) {
    ...
    const signature = await crypto.subtle.sign(
        { name: 'HMAC', saltLength: 20, hash: { name: 'SHA-1' } },
        key,
        encoder.encode(signedInfo)
    );
    return Buffer.from(signature).toString('base64')
...
SignedXml.prototype.calculateSignatureValue = async function(doc) {
  var signedInfoCanon = this.getCanonSignedInfoXml(doc)
  var signer = this.findSignatureAlgorithm(this.signatureAlgorithm)
  this.signatureValue = await signer.getSignature(signedInfoCanon, this.signingKey)
}
...
SignedXml.prototype.loadSignature = function(signatureNode) {
  if (typeof signatureNode === 'string') {
    this.signatureXml = signatureNode;
...
SignedXml.prototype.computeSignature = async function(xml, opts) {
  ...
  var dummyXML = new Dom().parseFromString(dummySignatureWrapper);
  var signatureDoc = dummyXML.documentElement.firstChild;
  ...
  await this.calculateSignatureValue(doc)
    ...
    for (var h in nodes) {
      ...
      res += "</" + prefix + "Transforms>"+
             "<" + prefix + "DigestMethod Algorithm=\"" + digestAlgorithm.getAlgorithmName() + "\" />"+
              "<" + prefix + "DigestValue>" + await digestAlgorithm.getHash(canonXml) + "</" + prefix + "DigestValue>"+
              "</" + prefix + "Reference>"
    }
  }
