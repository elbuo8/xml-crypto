...
function RSASHA1() {
  ...
  this.algorithm = 'RSA-SHA1';
  /**
  * Sign the given string using the given key
  *
  */
  this.getSignature = function(signedInfo, signingKey, cb) {
    return crypto.sign(this.algorithm, signedInfo, signingKey, cb);
  }
  ...
  *
  */
  this.verifySignature = function(str, key, signatureValue, cb) {
    return crypto.verify(this.algorithm, str, key, signatureValue, cb);
  }
...
function HMACSHA1() {
    ...
    this.verifySignature = function(str, key, signatureValue, cb) {
        ...
        return cb(null, res === signatureValue.toString('base64'));
    };
    ...
    this.getSignature = function(signedInfo, signingKey, cb) {
        ...
        return cb(null, verifier.digest('base64'));
    };
}
...
SignedXml.prototype.checkSignature = async function(xml) {
  ...
  return this.validateSignatureValue(doc);
}
...
SignedXml.prototype.calculateSignatureValue = async function(doc) {
  ...
  return new Promise((resolve, reject) => {
    signer.getSignature(signedInfoCanon, this.signingKey, (err, signatureValue) => {
      if (err) {
        return reject(err);
      }
...
SignedXml.prototype.loadSignature = function(signatureNode) {
  ...
  this.signatureValue = Buffer.from(
