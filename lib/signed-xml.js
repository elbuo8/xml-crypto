...
function SHA1() {
  ...
  this.getHash = async function(xml) {
    const buffer = await crypto.subtle.digest('SHA-1', encoder.encode(xml));
...
function SHA256() {
  ...
  this.getHash = async function(xml) {
    const buffer = await crypto.subtle.digest('SHA-256', encoder.encode(xml));
    return Buffer.from(buffer).toString('base64');
  }
...
function SHA512() {
  ...
  this.getHash = async function(xml) {
    const buffer = await crypto.subtle.digest('SHA-512', encoder.encode(xml));
    return Buffer.from(buffer).toString('base64');
  }
...
function RSASHA1() {
  ...
  */
  this.getSignature = async function(signedInfo, signingKey) {
    const keyData = utils.privatePemToKeyData(signingKey);
    const key = await crypto.subtle.importKey(
      'pkcs8',
        keyData,
      { name: ALGORITHM_NAME, hash: { name: 'SHA-1' } },
      false,
      ['sign']
    );

    const signature = await crypto.subtle.sign(
      { name: ALGORITHM_NAME, saltLength: 20 },
      key,
  ...
  */
  this.verifySignature = async function(str, key, signatureValue) {
    ...
    return await crypto.subtle.verify(
        { name: ALGORITHM_NAME, hash: { name: 'SHA-1' } },
        importedKey,
        utils.base64ToArrayBuffer(signatureValue),
        encoder.encode(str),
    );
  }
  ...
  */
  this.getSignature = async function(signedInfo, signingKey) {
    ...
    const key = await crypto.subtle.importKey(
        'pkcs8',
        keyData,
        { name: ALGORITHM_NAME, hash: { name: 'SHA-256' } },
        false,
        ['sign']
    );

    const signature = await crypto.subtle.sign(
        { name: ALGORITHM_NAME, saltLength: 32 },
        key,
        encoder.encode(signedInfo)
    );
    return Buffer.from(signature).toString('base64')
  ...
  */
  this.getSignature = async function(signedInfo, signingKey) {
    ...
    const key = await crypto.subtle.importKey(
        'pkcs8',
        keyData,
        { name: ALGORITHM_NAME, hash: { name: 'SHA-512' } },
        false,
        ['sign']
    );

    const signature = await crypto.subtle.sign(
        { name: ALGORITHM_NAME, saltLength: 64 },
        key,
        encoder.encode(signedInfo)
    );
    return Buffer.from(signature).toString('base64')
...
SignedXml.prototype.validateSignatureValue = async function(doc) {
  ...
  var res = await signer.verifySignature(signedInfoCanon, this.signingKey, this.signatureValue)
 ...
 *
 */
SignedXml.prototype.computeSignature = async function(xml, xpathToNodeBeforeSignature) {
  var doc = new Dom().parseFromString(xml)
  this.signatureXml = "<Signature xmlns=\"http://www.w3.org/2000/09/xmldsig#\">"                      
  
  var signedInfo = await this.createSignedInfo(doc);
  this.signatureXml += signedInfo;
  this.signatureXml += await this.createSignature(signedInfo);
 ...
 *
 */
SignedXml.prototype.createReferences = async function(doc) {
    ...
    for (var h in nodes) {      
      ...
      res += "</Transforms>"+
             "<DigestMethod Algorithm=\"" + digestAlgorithm.getAlgorithmName() + "\" />"+
              "<DigestValue>" + await digestAlgorithm.getHash(canonXml) + "</DigestValue>"+
              "</Reference>"
    }
  }
 ...
 *
 */
SignedXml.prototype.createSignedInfo = async function(doc) {
  ...
  res += await this.createReferences(doc)
  res += "</SignedInfo>"
  return res
}
 ...
 *
 */
SignedXml.prototype.createSignature = async function(signedInfo) {
  ...
  this.signatureValue = await signatureAlgorithm.getSignature(canonizedSignedInfo, this.signingKey)
